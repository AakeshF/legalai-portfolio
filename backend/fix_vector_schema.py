...
def fix_schema():
    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(text("ALTER TABLE search_cache ADD COLUMN hit_count INTEGER DEFAULT 0"))
            conn.execute(text("ALTER TABLE search_cache ADD COLUMN last_accessed TIMESTAMP"))
        ...
        try:
            conn.execute(text("ALTER TABLE embedding_models ADD COLUMN model_config TEXT"))
            conn.execute(text("ALTER TABLE embedding_models ADD COLUMN avg_encoding_time_ms REAL"))
            conn.execute(text("ALTER TABLE embedding_models ADD COLUMN total_documents_encoded INTEGER DEFAULT 0"))
        ...
        try:
            conn.execute(text("ALTER TABLE chunk_embeddings ADD COLUMN encoding_time_ms REAL"))

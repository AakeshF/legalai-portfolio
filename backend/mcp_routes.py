...
async def list_mcp_servers(
    ...
    try:
        ...
        return MCPServerListResponse(servers=servers, total_count=len(servers))
...
async def connect_mcp_server(
    ...
    try:
        ...
        success = await mcp_manager.connect_server(
            server_enum, current_user.organization_id, custom_config
        )
...
async def query_mcp_server(
    ...
    try:
        ...
        response = await mcp_manager.query_legal_mcp(
            server_enum, request.action, request.params, current_user
        )
        ...
        if (
            ...
            and response.metadata.get("reason") == "security_validation_failed"
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,

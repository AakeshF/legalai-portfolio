...
async def get_detailed_health(
    ...
    health_data["application"] = {
        "uptime_seconds": int((datetime.utcnow() - datetime(2024, 1, 1)).total_seconds()),  # TODO: Track actual start time
...
async def test_endpoints(
    ...
    test_results["endpoints"]["auth"] = {
        ...
        "refresh": _test_endpoint_availability("/api/auth/refresh", "POST", public=True)
    }
    
    # Calculate summary
    total_tests = sum(len(category) for category in test_results["endpoints"].values())
    passed_tests = sum(
        1 for category in test_results["endpoints"].values()
...
def _calculate_error_rate() -> float:
    ...
    total_requests = sum(
        count for key, count in metrics_collector.counters.items()
...
def _test_endpoint_availability(path: str, method: str, public: bool = False) -> Dict[str, Any]:

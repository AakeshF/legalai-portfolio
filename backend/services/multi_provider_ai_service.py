...
class ClaudeProvider(BaseAIProvider):
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        return {
            ...
            "features": ["legal_analysis", "document_review", "chat", "advanced_reasoning"]
        }
...
class LocalLLMProvider(BaseAIProvider):
    ...
    async def generate_response(self, messages: List[Dict[str, str]], **kwargs) -> str:
        if not self.is_available():
            raise Exception("Local LLM not available. Please ensure model is downloaded and MCP server is running.")
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        return {
            ...
            "available": self.is_available(),
            "features": ["legal_analysis", "document_review", "chat", "privacy", "offline"],
...
Always provide thorough, professional responses that would be valuable to an experienced attorney."""
        ...
        logger.info(f"Multi-provider AI service initialized. Default: {self.default_provider.value}")
        logger.info(f"Available providers: {[p.value for p, prov in self.providers.items() if prov.is_available()]}")
        ...
        try:
            ...
            return {
                ...
                "response_metrics": {
                    ...
                    "fallback_used": used_provider != (provider_enum or self.default_provider)
                }
            }
            ...
            return {
                ...
                "available_providers": [p["name"] for p in self.get_available_providers() if p["available"]],
                "fallback_attempted": True
            }
    
    def _get_user_friendly_error(self, error: Exception, attempted_provider: Optional[AIProvider] = None) -> Tuple[str, str]:
        ...
        # Network errors
        elif "timeout" in error_str or "connection" in error_str or error_type == "ConnectTimeout":
            return (
    ...
    def _extract_key_findings(self, response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(indicator in line_lower for indicator in ['found', 'identified', 'discovered']):

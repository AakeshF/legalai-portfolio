...
class LegalMCPSecurity:
    ...
    def validate_access(
        ...
        try:
            ...
            if not user.organization.is_active:
                self._log_access_denied(user, server_type, action, "Organization inactive")
                return False
            ...
            if not any(perm in user_perms for perm in required_perms):
                self._log_access_denied(user, server_type, action, "Insufficient permissions")
                return False
            
            # Check organization-specific access
            if not self._validate_organization_access(user, server_type, resource_id):
                self._log_access_denied(user, server_type, action, "Organization boundary violation")
                return False
    ...
    def _get_user_permissions(self, user: User) -> Set[str]:
        ...
        role_permissions = {
            "admin": {"court_access", "litigation", "client_access", "crm", "research_access", 
                     "document_access", "case_access", "billing_access", "compliance_access",
                     "corporate_access", "ip_access", "contract_access", "admin"},
            "attorney": {"court_access", "litigation", "client_access", "research_access",
                        "document_access", "case_access", "contract_access"},
    ...
    async def connect_server(
        ...
        try:
            ...
            if await server.connect():
                ...
                logger.info(f"Connected to {server_type.value} for org {organization_id}")
                return True
        ...
        try:
            ...
            if server_type not in self.servers:
                ...
                if not await self.connect_server(server_type, user.organization_id):
                    return MCPResponse(
                        success=False,
                        error=f"Server {server_type.value} not available"
                    )
    ...
    def get_server_status(self) -> Dict[str, Any]:
        ...
        for server_type in MCPServerType:
            ...
            status[server_type.value] = {
                ...
                "last_health_check": self.servers[server_type].last_health_check if connected else None
            }

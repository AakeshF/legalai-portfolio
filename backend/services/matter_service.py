...
class ConflictException(Exception):
    ...
    def __init__(self, conflicts: List[Dict[str, Any]]):
        ...
        super().__init__(
            f"Conflict of interest detected: {len(conflicts)} conflicts found"
        )
...
class MatterService:
    ...
    async def create_matter_with_conflict_check(
        self, matter_data: dict, organization_id: str
    ) -> Matter:
    ...
    async def _check_conflicts_via_mcp(
        self, client_name: str, opposing_parties: List[Dict], organization_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return await self._local_conflict_check(
                client_name, opposing_parties, organization_id
            )

    async def _local_conflict_check(
        self, client_name: str, opposing_parties: List[Dict], organization_id: str
    ) -> Dict[str, Any]:
    ...
    async def _sync_email_communications(self, matter: Matter):
        ...
        try:
            ...
            search_terms = (
                [matter.matter_name, matter.case_number]
                if matter.case_number
                else [matter.matter_name]
            )
    ...
    def _create_communication_from_email(self, matter: Matter, email: Dict):
        ...
        if not existing:
            communication = Communication(
                ...
                direction=(
                    "inbound"
                    if email.get("from") != matter.organization.billing_email
                    else "outbound"
                ),
        ...
        for source in mcp_sources:
            try:
                source_data = await self.mcp_manager.query_server(
                    source, "get_matter_data", {"matter_id": matter_id}
                )

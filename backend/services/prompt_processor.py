...
from models import (
    ...
    PromptAdminAction, ConsentRecord, ConsentType
)
...
class PromptProcessor:
        ...
        try:
            ...
            if requires_review:
                ...
                result = {
                    ...
                    "anonymization_applied": len(anonymization_result.sensitive_patterns) > 0,
    ...
    async def _detect_sensitive_data(
        ...
        return await self.sensitive_data_detector.analyze(
            prompt, db, user_id, org_id
        )
    ...
    async def _check_consent_requirements(
        ...
        sensitive_types = {
            ...
            "medical_record", "privileged_communication"
        }
        
        detected_sensitive = [
            item for item in sensitivity_result.get("detected_items", [])
        ...
        if consent and (not consent.expires_at or consent.expires_at > datetime.utcnow()):
            return False, None
        
        # Consent required
        return True, {
            "sensitive_data_types": list(set(item["type"] for item in detected_sensitive)),
    ...
    async def _estimate_wait_time(
        ...
        pending_count = db.query(PromptReviewQueue).filter(
    ...
    def _get_queue_position(
        ...
        earlier_entries = db.query(PromptReviewQueue).filter(
            PromptReviewQueue.organization_id == org_id,

...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        self.chunker = LegalDocumentChunker(
            chunk_size=512, chunk_overlap=100, min_chunk_size=100, max_chunk_size=1024
        )
        ...
        print(
            f"🤖 AI Service integration: {'Demo Mode' if self.ai_service.demo_mode else 'Active'}"
        )
    ...
    async def process_document(
        ...
        try:
            ...
            extraction_result = await self._extract_text_and_pages(
            ...
            if extract_metadata and not self.ai_service.demo_mode:
                ...
                metadata = await self.extract_legal_metadata(
                ...
                print(
                    f"📋 Metadata extracted: {metadata.get('document_type', 'unknown')} with {len(metadata.get('parties', []))} parties"
                )
    ...
    async def _extract_text_and_pages(
    ...
    async def _extract_pdf_text_and_pages(
        self, file_path: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"text": f"Error processing PDF: {str(e)}", "page_count": None}

    async def _extract_docx_text_and_pages(
        self, file_path: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"text": full_text, "page_count": estimated_pages}

        except Exception as e:
            print(f"❌ DOCX extraction error: {str(e)}")
            return {"text": f"Error processing DOCX: {str(e)}", "page_count": None}

    async def _extract_txt_text_and_pages(
        self, file_path: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for encoding in encodings:
                try:
                    ...
                    if content.strip():
                        ...
                        return {"text": content, "page_count": estimated_pages}
            ...
            return {"text": f"Error processing TXT: {str(e)}", "page_count": None}
    ...
    def _generate_summary(self, text: str, filename: str) -> str:
        ...
        if legal_score >= 3:
            ...
            if any(
                word in text_lower for word in ["lease", "rent", "tenant", "landlord"]
            ):
                doc_type = "Lease Agreement"
            elif any(
                word in text_lower
                for word in ["employment", "employee", "employer", "salary"]
            ):
                doc_type = "Employment Contract"
            elif any(
                word in text_lower for word in ["non-disclosure", "confidential", "nda"]
            ):
                doc_type = "Non-Disclosure Agreement"
            elif any(
                word in text_lower for word in ["purchase", "sale", "buyer", "seller"]
            ):
        ...
        preview = (
            sentences[0][:200] + "..."
            if sentences
            else "No meaningful content preview available."
        )
...
"""
        ...
        try:
            ...
            response = await self.ai_service._call_ai_api(
                [
                    {
                        "role": "system",
                        "content": "You are a legal document analyzer. Extract information and return valid JSON only.",
                    },
                    {
                        "role": "user",
                        "content": metadata_prompt + text[:8000],
    ...
    def _fallback_metadata_extraction(self, text: str) -> Dict[str, Any]:
        ...
        metadata["monetary_amounts"] = [
            {"description": "mentioned", "amount": amt} for amt in amounts[:5]
        ]
    ...
    async def _extract_pdf_streaming(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"✅ Extracted {len(full_text)} characters from large PDF (capped at {max_chars})"
            )
            ...
            return {"text": f"Error processing large PDF: {str(e)}", "page_count": None}

    async def _generate_chunks_and_embeddings(
        self, db, document: Document, text: str, metadata: Optional[Dict] = None
    ):
        """Generate chunks and embeddings for semantic search"""
        try:
            ...
            embedding_model = (
                ...
                .filter(EmbeddingModel.name == self.embedding_service.model_name)
                .first()
            )
            ...
            chunks = self.chunker.chunk_document(
                text=text,
                document_type=(
                    metadata.get("document_type", "general") if metadata else "general"
                ),
                metadata=metadata,
            )

...
class AnonymizationService:
    def __init__(self):
        ...
        try:
            ...
            print(
                f"Warning: Invalid encryption key. Generated new key for this session."
            )
    ...
    def _get_patterns(
        self, db: Session, user_id: Optional[int], org_id: Optional[int]
    ) -> Dict[str, str]:
    ...
    def _should_anonymize(
        self, pattern: SensitivePattern, rules: List[AnonymizationRule]
    ) -> bool:
        """Check if pattern should be anonymized based on rules"""
        ...
        for rule in sorted(
            applicable_rules, key=lambda x: x.user_id is not None, reverse=True
        ):
    ...
    def _check_consent_requirements(
        self, patterns: List[SensitivePattern], rules: List[AnonymizationRule]
    ) -> bool:

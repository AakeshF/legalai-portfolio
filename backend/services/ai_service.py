...
class AIService:
    ...
    def __init__(self):
        ...
        self.is_demo_mode = settings.demo_mode or os.getenv("DEMO_MODE", "false").lower() == "true"
    ...
    async def process_chat_message(
        ...
        try:
            ...
            if self.demo_mode or self.is_demo_mode:
                ...
                if self.is_demo_mode and self.api_key and self.api_key.startswith("sk-"):
            ...
            if query_type in ['parties', 'dates', 'amounts', 'obligations'] and documents:
                ...
                if metadata_response:
                    ...
                    estimated_tokens_saved = self._estimate_tokens_saved(message, documents, query_type)
...
{self.system_prompts[prompt_key]}"""
    ...
    async def compare_documents(self, documents: List[Any], comparison_type: str = "general") -> Dict[str, Any]:
...
Focus on legally significant differences."""
    ...
    async def extract_key_terms(self, document: Any) -> Dict[str, List[Any]]:
        ...
        extracted = {
            ...
            "case_citations": re.findall(self.legal_patterns["case_citations"], content),
    ...
    async def _extract_key_legal_terms(self, content: str) -> List[str]:
        ...
        if self.demo_mode:
            ...
            return ["agreement", "liability", "indemnification", "breach", "damages", 
                   "termination", "jurisdiction", "confidentiality"]
        ...
        try:
            ...
            response = await self._call_[ai-provider]_api(messages)
    ...
    async def _detect_document_type(self, document: Any) -> str:
        ...
        content_lower = document.extracted_content.lower()[:2000]  # Check first 2000 chars
    ...
    async def _call_ai_api(self, messages: List[Dict[str, str]]) -> str:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            if response.status_code != 200:
                ...
                raise Exception(f"API call failed: {response.status_code} - {error_detail}")
    ...
    async def _demo_response(self, message: str, documents: List[Any], error: str = None) -> Dict[str, Any]:
...
- Bulk document processing"""
        ...
        elif any(word in message_lower for word in ["analyze", "analysis", "review", "check"]):
            response = legal_responses["analysis"]
        elif any(word in message_lower for word in ["feature", "can you", "what do", "capabilities"]):
        ...
        return {
            ...
            "sources": [{"document_id": doc.id, "document_name": doc.filename, "relevance": "available"} for doc in documents[:3]],
            "timestamp": datetime.utcnow(),
            "model": "demo-mode",
            "analysis_type": "demo",
    ...
    def _estimate_tokens_used(self, messages: List[Dict[str, str]], response: str) -> int:
    ...
    def _estimate_tokens_saved(self, message: str, documents: List[Any], query_type: str) -> int:
        ...
        tokens_that_would_be_used += len(self.system_prompts.get("legal_assistant", "")) // 4

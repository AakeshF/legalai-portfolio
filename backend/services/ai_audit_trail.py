...
class AIAuditTrail:
    ...
    def log_ai_request(
        ...
        audit_log = AIAuditLog(
            ...
            estimated_cost=self._estimate_cost(provider, metadata.get("tokens_used", 0)),
    ...
    def get_audit_log(self, request_id: str) -> Optional[Dict[str, Any]]:
        ...
        details = self.db.query(AIDecisionDetail).filter_by(
            audit_log_id=log.id
        ).all()
    ...
    def get_usage_analytics(
        ...
        for provider, stats in by_provider.items():
            ...
            if provider_logs:
                avg_time = sum(log.response_time_ms or 0 for log in provider_logs) / len(provider_logs)
    ...
    def apply_retention_policy(self, dry_run: bool = True) -> Dict[str, int]:
        ...
        expired_logs = self.db.query(AIAuditLog).filter(

...
class EmbeddingService:
            ...
            for i, (text, embedding) in enumerate(zip(batch, embeddings)):
                ...
                if self.cache_embeddings and metadata:
                    ...
                    cache_key = self._get_cache_key(
                        text,
                        metadata[meta_idx] if meta_idx < len(metadata) else None
                    )
                    self.cache[cache_key] = result
    ...
    async def _generate_local_embeddings_batch(self, texts: List[str]) -> List[np.ndarray]:
    ...
    async def _generate_external_embeddings_batch(self, texts: List[str]) -> List[List[float]]:

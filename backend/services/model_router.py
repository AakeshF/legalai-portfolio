...
from models import User, Organization, APIKeyStore, AIAuditLog, PromptLog, PromptStatus
...
class ModelRouter:
    ...
    async def route_request(
        ...
        provider_order = self._determine_provider_order(user, org, preferred_provider)
        ...
        for provider_name in provider_order:
            try:
                ...
                # Get API key
                api_key = await self._get_api_key(db, provider_name, user_id, org_id)
    ...
    def _determine_provider_order(
        self, user: User, org: Organization, preferred_provider: Optional[str]
    ) -> List[str]:
        ...
        # 2. User preference
        if (
            user.ai_provider_preference
            and user.ai_provider_preference in self.providers
        ):
        ...
        if (
            hasattr(org, "preferred_ai_provider")
            and org.preferred_ai_provider in self.providers
        ):
    ...
    async def _get_api_key(
        self, db: Session, provider: str, user_id: str, org_id: str
    ) -> Optional[str]:
        ...
        # Try user key first
        user_key = await self.api_key_manager.get_api_key(db, provider, user_id=user_id)
        if user_key:
            return user_key

        # Try organization key
        org_key = await self.api_key_manager.get_api_key(db, provider, org_id=org_id)
        if org_key:
            return org_key
    ...
    def _get_model_config(
        ...
        max_tokens = min(4000, getattr(org, "ai_max_tokens_per_request", 4000))

        rate_limit = min(60, getattr(org, "ai_rate_limit_per_minute", 10))
    ...
    async def _check_rate_limit(self, provider: str, org_id: str) -> bool:
        ...
        if key not in self._rate_limiters:
            self._rate_limiters[key] = {"requests": [], "limit": 10}
        ...
        # Remove old requests
        limiter["requests"] = [
            req for req in limiter["requests"] if now - req < timedelta(minutes=1)
        ]
    ...
    async def _log_request(
        ...
        if prompt_log_id:
            prompt_log = (
                db.query(PromptLog).filter(PromptLog.id == prompt_log_id).first()
            )
            if prompt_log:

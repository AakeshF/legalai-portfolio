...
from models import (
    User, Organization, Document, PromptLog, PromptStatus,
    ChatSession, ChatMessage, AIAuditLog, ConsentType
)
...
class IntegratedAIAssistant:
        ...
        try:
            ...
            if security_decision.action == SecurityAction.REQUIRE_CONSENT:
                return {
                    ...
                    "required_consents": [c.value for c in security_decision.required_consents],
                    "classification": security_decision.classification.value
                }
            ...
            return {
                ...
                "metadata": {
                    ...
                    "anonymization_applied": prompt_result.get("anonymization_applied", False),
                    "classification": security_decision.classification.value,
    ...
    async def _post_process_response(
        ...
        if security_decision.redaction_required:
            ...
            if sensitivity_result.get("overall_sensitivity", 0) > 0.5:
                ...
                anonymization_result = self.prompt_processor.anonymization_service.anonymize_text(
                    response, db, user_id, org_id
                )
    ...
    async def get_usage_statistics(
        ...
        ).group_by(AIAuditLog.model_used).all()

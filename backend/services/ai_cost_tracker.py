...
class AICostTracker:
    ...
    async def check_budget_before_request(
        self, organization_id: str, provider: str, estimated_tokens: int
    ) -> Dict[str, Any]:
        ...
        provider_rates = self.provider_costs.get(
    ...
    async def record_usage_cost(
        self, organization_id: str, provider: str, tokens_used: int, request_id: str
    ) -> float:
        ...
        cost = (input_tokens / 1_000_000) * provider_rates["input"] + (
            output_tokens / 1_000_000
        ) * provider_rates["output"]
        ...
        if org:
            ...
            if org.ai_monthly_budget and org.ai_cost_alerts_enabled:
                usage_percentage = (
                    org.ai_current_month_cost / org.ai_monthly_budget
                ) * 100
                ...
                for threshold in alert_thresholds:
                    if (
                        ...
                        and usage_percentage < threshold + 5
                    ):
                        asyncio.create_task(
    ...
    async def get_current_month_cost(self, organization_id: str) -> float:
        ...
        total_cost = (
            self.db.query(func.sum(AIAuditLog.estimated_cost))
            .filter(
    ...
    async def get_cost_breakdown(
        ...
        provider_costs = (
            ...
            .group_by(AIAuditLog.provider_used)
            .all()
        )
        ...
        for provider, cost, count, tokens in provider_costs:
            breakdown[provider] = {
                ...
                "avg_cost_per_request": (
                    round((cost or 0) / count, 4) if count > 0 else 0
                ),
            }
        ...
        return {
            ...
            "usage_percentage": (
                round((total_cost / org.ai_monthly_budget) * 100, 1)
                if org and org.ai_monthly_budget
                else None
            ),
    ...
    async def get_cost_trends(
        ...
        daily_costs = (
            ...
            .group_by(func.date(AIAuditLog.request_timestamp))
            .all()
        )
    ...
    async def _send_budget_alert(
        self, org: Organization, current_cost: float, projected_cost: float
    ):
        ...
        try:
            await self.email_service.send_email(
                to=org.billing_email, subject=subject, body=body, html=True
            )
    ...
    async def _send_usage_alert(
        self, org: Organization, usage_percentage: float, threshold: int
    ):
    ...
    def update_budget_settings(
        ...
        return {
            ...
            "usage_percentage": (
                round((org.ai_current_month_cost / org.ai_monthly_budget) * 100, 1)
                if org.ai_monthly_budget
                else 0
            ),
        }

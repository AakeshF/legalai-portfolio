...
class MCPCacheMonitor:
    ...
    async def track_query_pattern(
        self, query_type: str, response_time: float, cached: bool, cache_name: str
    ):
    ...
    async def analyze_cache_performance(
        self, cache_name: Optional[str] = None, timeframe_hours: int = 24
    ) -> CacheAnalytics:
    ...
    async def _calculate_hit_rate(
        self, db: AsyncSession, cache_name: Optional[str], cutoff: datetime
    ) -> float:
        ...
        total_result = await db.execute(
            select(func.count(MCPCacheMetrics.id)).where(and_(*conditions))
        )
        ...
        hit_result = await db.execute(
            select(func.count(MCPCacheMetrics.id)).where(and_(*hit_conditions))
        )
    ...
    async def _analyze_miss_patterns(
        ...
        result = await db.execute(
            select(
                MCPCacheMetrics.key, func.count(MCPCacheMetrics.id).label("miss_count")
            )
    ...
    async def _get_cache_size(self, db: AsyncSession, cache_name: Optional[str]) -> int:
    ...
    async def _calculate_eviction_rate(
        ...
        # Get evictions
        eviction_conditions = conditions + [
            MCPCacheMetrics.event_type == CacheEventType.EVICTION
        ]
        eviction_result = await db.execute(
            select(func.count(MCPCacheMetrics.id)).where(and_(*eviction_conditions))
        )
    ...
    async def get_cache_health_score(
        ...
        while True:
            try:
                ...
                for cache_name in self.cache_stats.keys():
                    analytics = await self.analyze_cache_performance(
                        cache_name, timeframe_hours=1
                    )

                    # Log warnings if needed
                    if analytics.hit_rate < 0.5:
                        logger.warning(
                            f"Low cache hit rate for {cache_name}: {analytics.hit_rate:.1%}"
                        )

                    if analytics.eviction_rate > 0.2:
                        logger.warning(
                            f"High eviction rate for {cache_name}: {analytics.eviction_rate:.1%}"
                        )
    ...
    async def _periodic_cleanup(self):
        ...
        while True:
            try:
                ...
                self.cache_events = [
                    e for e in self.cache_events if e["timestamp"] > cutoff
                ]

...
class LocalEmailService:
    ...
    def __init__(self):
        ...
        logger.info(f"LocalEmailService initialized - emails will be saved to {self.email_dir}")
    ...
    async def send_email(self, to: List[str], subject: str, body: str, html_body: Optional[str] = None):
        ...
        filename = f"{timestamp.replace(':', '-')}_{subject[:30].replace(' ', '_')}.json"
    ...
    async def send_welcome_email(self, user_email: str, user_name: str, organization: str):
...
"""
    ...
    async def send_document_processed(self, user_email: str, document_name: str, summary: str):

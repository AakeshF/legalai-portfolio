...
def _calculate_cost_savings(self, tokens_saved: int) -> str:
    ...
    cost = (
        prompt_tokens / 1000 * self.token_costs["prompt"]
        ...
        try:
            ...
            if metadata.get("parties"):
                for party in metadata["parties"]:
                    party_name = (
                        party.get("name", "") if isinstance(party, dict) else str(party)
                    )
            ...
            if (
                ...
                and metadata["document_type"] in response_lower
            ):
                context_types.append("document_type")

            if (
                metadata.get("jurisdiction")
                and metadata["jurisdiction"].lower() in response_lower
            ):
...
def _identify_context_types(self, context: str) -> List[str]:
    ...
    if (
        "parties" in context_lower
        or "between" in context_lower
        or "party" in context_lower
    ):
        types.append("parties_information")

    if (
        "date" in context_lower
        or "deadline" in context_lower
        or "effective" in context_lower
    ):
    ...
    if (
        "obligation" in context_lower
        or "shall" in context_lower
        or "must" in context_lower
    ):
        ...
        "cache_efficiency": {
            "tokens_saved_percentage": (
                (
                    ...
                    / (
                        self.total_tokens_saved
                        + self.last_token_usage.get("total_tokens", 1)
                    )
                )
                * 100
...
def _calculate_request_cost(self, usage: Dict[str, int]) -> float:
    ...
    completion_cost = (
        usage.get("completion_tokens", 0) / 1000 * self.token_costs["completion"]
    )

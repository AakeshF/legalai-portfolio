...
class ProviderHealthMonitor:
    ...
    async def check_provider_health(
        ...
        try:
            ...
            self._update_health_status(
                provider=provider, status="down", response_time=None, error=str(e)
            )
            return {
                "provider": provider,
                "status": "down",
    ...
    def _update_health_status(
        ...
        health_record = (
            self.db.query(ProviderHealthStatus).filter_by(provider=provider).first()
        )
    ...
    def get_provider_statistics(self, provider: str, hours: int = 24) -> Dict[str, Any]:
        ...
        if not health_record:
            return {"provider": provider, "status": "unknown", "statistics": None}
    ...
    async def start_monitoring(self, api_keys_callback, interval: int = 300):

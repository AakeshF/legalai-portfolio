    ...
    async def is_triggered(self, context: Dict[str, Any]) -> bool:
        ...
        for server_name, health in health_status.items():
            if health.get("status") == "error":
                ...
                if last_check:
                    downtime = (
                        datetime.utcnow() - datetime.fromisoformat(last_check)
                    ).seconds
...
class MCPAlertManager:
        ...
        for rule in self.alert_rules:
            try:
                if await rule.is_triggered(context):
                    ...
                    alert_key = (
                        f"{rule.name}:{context.get('triggered_server', 'global')}"
                    )
    ...
    async def _gather_context(self) -> Dict[str, Any]:
        ...
        # Get cache analytics
        cache_analytics = await cache_monitor.analyze_cache_performance(
            timeframe_hours=1
        )
    ...
    async def get_alert_summary(self) -> Dict[str, Any]:
        ...
        async for db in get_db():
            ...
            result = await db.execute(
                select(MCPAlert.severity, func.count(MCPAlert.id).label("count"))
            ...
            result = await db.execute(
                select(func.count(MCPAlert.id)).where(
                    and_(MCPAlert.resolved == True, MCPAlert.resolved_at > cutoff)
                )
            )

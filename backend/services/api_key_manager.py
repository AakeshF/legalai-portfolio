...
class APIKeyStore(Base):
    ...
    provider = Column(String(50), nullable=False)  # claude, openai, gemini, [ai-provider]
...
class APIKeyManager:
    ...
    def __init__(self, db: Session, master_key: Optional[str] = None):
        self.db = db
        self.master_key = master_key or os.getenv("API_KEY_MASTER_SECRET", self._generate_master_key())
    ...
    def _initialize_cipher(self) -> Fernet:
        ...
        salt_str = os.getenv("API_KEY_SALT", base64.urlsafe_b64encode(os.urandom(16)).decode())
    ...
    def store_api_key(
        ...
        existing = self.db.query(APIKeyStore).filter_by(
    ...
    def get_api_key(self, organization_id: int, provider: str) -> Optional[str]:
        ...
        key_record = self.db.query(APIKeyStore).filter_by(
            organization_id=organization_id,
            provider=provider,
            is_active=True
        ).first()
        ...
        try:
            # Decrypt the key
            decrypted_key = self._cipher_suite.decrypt(key_record.encrypted_key.encode()).decode()
    ...
    def list_api_keys(self, organization_id: int) -> List[Dict[str, Any]]:
        ...
        keys = self.db.query(APIKeyStore).filter_by(
            organization_id=organization_id,
            is_active=True
        ).all()
    ...
    def revoke_api_key(self, organization_id: int, provider: str) -> bool:
        ...
        if key_record:
            ...
            logger.info(f"API key revoked for org {organization_id}, provider {provider}")
            return True

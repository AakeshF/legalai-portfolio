...
from models import (
    User, Organization, Document, PromptLog, ConsentRecord,
    AIAuditLog, ConsentType, ConsentScope
)
...
class SecurityEnforcementService:
    ...
    def _classify_data(self, sensitivity_analysis: Dict[str, Any]) -> DataClassification:
        ...
        if "privileged" in categories or "attorney_client" in categories.get("legal", []):
    ...
    def _make_security_decision(
        ...
        if required_consents and not consent_granted:
            ...
            reasons.append(f"Missing consent for: {', '.join(c.value for c in required_consents)}")
    ...
    async def generate_compliance_report(
        ...
        blocked_requests = sum(
            ...
            if log.decision_summary and 
            json.loads(log.decision_summary).get("action") == "block"
        )
        
        # Get consent compliance
        consent_manager = ConsentManager(db)
        consent_report = consent_manager.get_compliance_report(org_id, start_date, end_date)
        ...
        prompt_reviews = db.query(PromptLog).filter(
    ...
    def _calculate_compliance_score(
        ...
        if review_stats["total_prompts"] > 0:
            review_rate = (
                ...
                review_stats["total_prompts"] * 100
            )
    ...
    async def enforce_data_retention_policy(
        ...
        pref = db.query(ConsentPreference).filter(
            ConsentPreference.organization_id == org_id
        ).first()
        ...
        old_prompts = db.query(PromptLog).filter(
            PromptLog.organization_id == org_id,
            PromptLog.created_at < cutoff_date
        ).all()
        ...
        # Mark old audit logs
        old_audits = db.query(AIAuditLog).filter(
            AIAuditLog.organization_id == org_id,

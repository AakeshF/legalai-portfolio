...
from services.ollama_service import OllamaService as AIService, LegalDocumentType, RiskLevel
...
class LocalLLMClient:
    ...
    def __init__(self, endpoint: str = None, model: str = None, api_key: Optional[str] = None):
        self.endpoint = endpoint or os.getenv("LOCAL_LLM_ENDPOINT", "http://localhost:11434")
        ...
        logger.info(f"LocalLLMClient initialized - Endpoint: {self.endpoint}, Model: {self.model}")
...
class HybridAIService(AIService):
        ...
        try:
            ...
            if query_type in ['parties', 'dates', 'amounts', 'obligations'] and documents:
                ...
                if metadata_response:
                    return self._add_performance_metrics(metadata_response, start_time, backend)
            ...
            if backend == AIBackend.LOCAL and organization and organization.ai_fallback_enabled:
                ...
                try:
                    ...
                    return self._add_performance_metrics(response, start_time, AIBackend.CLOUD)
    ...
    async def _process_with_local_llm(
        ...
        document_context = self._build_enhanced_document_context(documents, self._classify_query(message))
    ...
    async def _process_with_cloud(
        ...
        return await super().process_chat_message(message, documents, chat_history, analysis_type)
    ...
    def _add_performance_metrics(self, response: Dict[str, Any], start_time: float, backend: AIBackend) -> Dict[str, Any]:
    ...
    def _extract_sources_from_text(self, answer: str, documents: List[Any]) -> List[str]:
        ...
        return {
            "backends": {
                ...
                "local": {
                    ...
                    "endpoint": self._local_client.endpoint if self._local_client else None,

...
class SemanticSearchEngine:
    ...
    async def search(
        ...
        try:
            ...
            keyword_results = await self._keyword_search(
                db, query, organization_id, document_ids, document_types, top_k * 2
            )

            # Combine and re-rank results
            final_results = self._combine_results(
                vector_results, keyword_results, similarity_threshold, top_k
            )
    ...
    async def _vector_search(
        ...
        params = {"query_embedding": embedding_str, "org_id": organization_id}
        ...
        for row in result:
            ...
            vector_results.append((row.chunk_id, row.content, row.similarity, metadata))
    ...
    async def _keyword_search(
        ...
        for chunk in results:
            ...
            matches = sum(content_lower.count(term) for term in search_terms)
            ...
            keyword_results.append((chunk.id, chunk.content, keyword_score, metadata))
    ...
    async def find_similar_documents(
        self, document_id: str, organization_id: str, top_k: int = 5
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            similar_docs = sorted(
                doc_scores.values(), key=lambda x: x["avg_score"], reverse=True
            )

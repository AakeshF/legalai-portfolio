...
class EnhancedDocumentProcessor(DocumentProcessor):
    ...
    def _is_court_document(self, document: ProcessedDocument) -> bool:
        ...
        court_indicators = [
            'court', 'judge', 'docket', 'case no', 'plaintiff', 'defendant',
            'motion', 'complaint', 'answer', 'order', 'judgment', 'verdict',
            'filed', 'civil action', 'criminal case'
        ]
        
        text_lower = document.text.lower()
        score = sum(1 for indicator in court_indicators if indicator in text_lower)
        
        return score >= 3 or document.detected_type in ['motion', 'complaint', 'court_filing']
    ...
    def _detect_document_type(self, text: str) -> str:
        ...
        if 'complaint' in text_lower and 'plaintiff' in text_lower:
            return 'complaint'
        elif 'motion' in text_lower and ('court' in text_lower or 'judge' in text_lower):
            return 'motion'
    ...
    async def _enhance_court_filing(self, document: ProcessedDocument) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.court_mcp:
                result = await self.court_mcp.query("analyze_filing", {
    ...
    def _extract_case_references(self, text: str) -> List[str]:
        ...
        case_number_pattern = re.compile(r'(?:Case No\.|No\.|Case Number)\s*:?\s*([A-Z0-9\-]+)')
        ...
        try:
            ...
            for citation in citations:
                ...
                validated.append({
                    ...
                    "type": "case_law" if " v. " in citation or " vs. " in citation else "case_number"
                })
    ...
    def _extract_party_names(self, text: str) -> List[Dict[str, str]]:
        ...
        for match in matches:
            ...
            if party_name and len(party_name) > 3:
                ...
                before_text = text[max(0, text.find(party_name) - 20):text.find(party_name)]
                party_type = "unknown"
        ...
        caption_pattern = re.compile(r'([A-Z][A-Za-z\s&,.\'-]+?)\s+v\.?\s+([A-Z][A-Za-z\s&,.\'-]+?)(?:\s*\n|$)')
        caption_matches = caption_pattern.findall(text[:1000])  # Check first part of document
    ...
    async def _check_party_conflicts(self, parties: List[Dict[str, str]], organization_id: str) -> Dict[str, Any]:
    ...
    async def _interpret_deadlines(self, deadline_refs: List[Dict[str, str]], document_date: datetime) -> Dict[str, Any]:
...
class MCPDocumentClassifier:
    ...
    async def classify_with_court_knowledge(
        ...
        if jurisdiction:
            async with self.court_mcp:
                types_result = await self.court_mcp.query("get_document_types", {
                    "jurisdiction": jurisdiction
                })
        ...
        classification = await self._ai_classify(
            document_text,
            valid_types=court_types or self._get_default_types()
        )
        
        return classification
        
    async def _ai_classify(self, document_text: str, valid_types: List[str]) -> Dict[str, Any]:
    ...
    def _get_default_types(self) -> List[str]:
        ...
        return [
            "complaint", "answer", "motion", "brief", "order",
            "contract", "lease", "discovery", "notice", "other"
        ]
...
class DocumentSearchService:
    ...
    async def _extract_legal_concepts(self, query: str) -> Dict[str, Any]:
        ...
        # Extract legal terms
        legal_terms = [
            "negligence", "contract", "breach", "liability", "damages",
            "plaintiff", "defendant", "motion", "discovery", "jurisdiction"
        ]
        query_lower = query.lower()
        concepts["legal_terms"] = [term for term in legal_terms if term in query_lower]
        
        # Extract party names (simplified)
        party_pattern = re.compile(r'([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+v\.?\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)')

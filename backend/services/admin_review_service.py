...
from models import (
    User, Organization, PromptLog, PromptStatus, PromptReviewQueue,
    PromptAdminAction
)
...
class AdminReviewService:
    ...
    async def get_pending_prompts(
        ...
        query = db.query(
            ...
            PromptLog, PromptReviewQueue.prompt_log_id == PromptLog.id
        ).join(
            User, PromptLog.user_id == User.id
        ).filter(
        ...
        for queue_entry, prompt_log, user in results:
            pending_prompts.append({
                ...
                "due_by": queue_entry.due_by.isoformat() if queue_entry.due_by else None
            })
    ...
    async def assign_prompt(
        ...
        queue_entry = db.query(PromptReviewQueue).filter(
    ...
    async def approve_prompt(
        ...
        if edited_prompt and edited_prompt != prompt_log.redacted_prompt:
            ...
            admin_action = PromptAdminAction(
                ...
                action_details={
                    "reason": "Admin edited prompt during approval"
                },
        ...
        approval_action = PromptAdminAction(
            ...
            action_details={
                "notes": notes,
                "edited": edited_prompt is not None
            },
            timestamp=datetime.utcnow()
        )
    ...
    async def get_analytics(
        ...
        for status in PromptStatus:
            count = db.query(PromptLog).filter(
                PromptLog.organization_id == org_id,
        ...
        # Average review time
        reviewed_prompts = db.query(PromptLog).filter(
            PromptLog.organization_id == org_id,
        ...
        # Queue statistics
        current_queue_size = db.query(PromptReviewQueue).filter(
            PromptReviewQueue.organization_id == org_id
        ).count()
        
        queue_by_priority = {}
        for priority in ["critical", "high", "medium", "low"]:
            count = db.query(PromptReviewQueue).filter(
                PromptReviewQueue.organization_id == org_id,
        ...
        admin_actions = db.query(
            ...
            User, PromptAdminAction.admin_id == User.id
        ).join(
            PromptLog, PromptAdminAction.prompt_log_id == PromptLog.id
        ).filter(
            PromptLog.organization_id == org_id,
    ...
    async def escalate_overdue_prompts(self, db: Session):
        ...
        overdue_prompts = db.query(PromptReviewQueue).filter(
        ...
        for prompt in overdue_prompts:
            ...
            org = db.query(Organization).filter(
                Organization.id == prompt.organization_id
            ).first()

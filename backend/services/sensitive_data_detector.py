...
class SensitiveDataDetector:
    ...
    def __init__(self):
        ...
        self.categories = {
            ...
            "financial": ["credit_card", "bank_account", "ein", "currency", "financial_record"],
            "medical": ["medical_record", "diagnosis", "treatment", "medication"],
            "privileged": ["privileged_communication", "confidential_memo", "settlement_terms"]
        }
    ...
    def _detect_patterns(self, text: str) -> List[SensitiveDataItem]:
        ...
        for pattern_type, regex in patterns.items():
            for match in re.finditer(regex, text, re.IGNORECASE):
                ...
                detected.append(SensitiveDataItem(
                    type=pattern_type,
                    text=match.group(),
                    start=match.start(),
                    end=match.end(),
    ...
    def _detect_contextual_sensitive(self, text: str) -> List[SensitiveDataItem]:
        ...
        privilege_patterns = [
            ...
            (r"(?:confidential memorandum|privileged communication)", "privileged_communication"),
        ...
        for pattern, data_type in privilege_patterns:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                ...
                confidence = 0.7
                if any(word in context.lower() for word in ["confidential", "privileged", "not for disclosure"]):
                    confidence = 0.9
                    ...
                    if result["score"] > 0.7:
                        ...
                        if data_type:
                            detected.append(SensitiveDataItem(
                                ...
                                end=chunk_start + result.get("end", len(chunk_text)),
    ...
    def _merge_detections(self, items: List[SensitiveDataItem]) -> List[SensitiveDataItem]:
    ...
    def _generate_summary(self, items: List[SensitiveDataItem]) -> str:
        ...
        return f"Detected: {', '.join(summary_parts[:3])}" + (" and more" if len(summary_parts) > 3 else "")

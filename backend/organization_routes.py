...
from schemas import (
    OrganizationResponse, OrganizationUpdate, UserInvite, 
    UserResponse, OrganizationUsersResponse
)
import logging
...
async def get_organization_details(
    ...
    logger.info(f"Organization details requested", extra={
    ...
    return OrganizationResponse(
        ...
        storage_used_mb=sum(doc.file_size for doc in current_org.documents) / (1024 * 1024)
    )
...
async def update_organization(
    ...
    logger.info(f"Organization updated", extra={
        "organization_id": current_org.id,
...
async def invite_user(
    ...
    logger.info(f"New user invited - Email: {invite.email}, Temp Password: {temp_password}")
    
    logger.info(f"User invited to organization", extra={
        "organization_id": current_org.id,
...
async def remove_user(
    ...
    logger.info(f"User removed from organization", extra={
        "organization_id": current_org.id,
        "removed_by": current_user.id,
        "removed_user": user_id
    })
    
    return {"message": f"User {user_to_remove.email} has been removed from the organization"}
...
async def update_user_role(
    ...
    logger.info(f"User role updated", extra={
        "organization_id": current_org.id,
        "updated_by": current_user.id,

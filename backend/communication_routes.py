...
from models import (
    ClientCommunication, CommunicationFollowUp, CommunicationTemplate,
    Matter, User, Organization
)
...
async def log_communication(
    ...
    try:
        ...
        result = await communication_mcp.query("log_communication", {
...
async def search_communications(
    ...
    try:
        ...
        if request.communication_type:
            query = query.filter(ClientCommunication.communication_type == request.communication_type)
...
async def generate_privilege_log(
    ...
    try:
        ...
        # Generate via MCP
        result = await communication_mcp.query("generate_privilege_log", {
            "matter_id": request.matter_id,
            "date_range": {
                "start": request.start_date.isoformat(),
                "end": request.end_date.isoformat()
            } if request.start_date and request.end_date else None
        })
...
async def create_follow_up(
    ...
    try:
        ...
        communication = db.query(ClientCommunication).filter(
        ...
        result = await communication_mcp.query("set_follow_up", {
...
async def get_communication_stats(
    ...
    try:
        ...
        result = await communication_mcp.query("get_communication_stats", {
            "matter_id": matter_id,
            "date_range": {
...
async def get_follow_ups(
    ...
    try:
        ...
        if assigned_to_me:
            query = query.filter(CommunicationFollowUp.assigned_to_id == current_user.id)
...
async def mark_follow_up_complete(
    ...
    organization_id = current_org.id
    try:
        follow_up = db.query(CommunicationFollowUp).filter(
...
async def _process_bulk_import(
    ...
    try:
        if import_data["source"] == "email":
            result = await communication_mcp.query("sync_emails", {

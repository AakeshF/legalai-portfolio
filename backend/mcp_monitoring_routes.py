...
async def get_mcp_analytics(
    timeframe: str = Query("24h", description="Timeframe for analytics (e.g., 1h, 24h, 7d)"),
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def get_performance_summary(
    ...
    try:
        ...
        for server, perf in analytics.get("server_performance", {}).items():
            avg_response_time += perf.get("avg_response_time", 0) * perf.get("total_requests", 0)
            total_requests += perf.get("total_requests", 0)
            total_errors += int(perf.get("error_rate", 0) * perf.get("total_requests", 0) / 100)
...
async def export_metrics(
    ...
    try:
        ...
        if format == "csv":
            ...
            writer.writerow(["Server", "Total Requests", "Avg Response Time", "Success Rate", "Error Rate"])

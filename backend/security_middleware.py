...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def _add_security_headers(self, response: Response):
        ...
        # Content Security Policy
        csp_header = "; ".join(
            [f"{k} {v}" for k, v in self.config.csp_directives.items()]
        )
...
class CSRFProtectionMiddleware(BaseHTTPMiddleware):
    ...
    def _generate_csrf_token(self) -> str:
        ...
        self.csrf_tokens = {k: v for k, v in self.csrf_tokens.items() if v > cutoff}
...
class IPWhitelistMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, config: SecurityConfig = None, geoip_db_path: Optional[str] = None
    ):
        super().__init__(app)
        self.config = config or SecurityConfig()
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not client_ip:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Client IP required"
            )
        ...
        if self.config.enable_geo_blocking and self.geoip_reader:
            ...
            if country:
                ...
                if country in self.config.blocked_countries:
                    self._log_blocked_access(
                        request, client_ip, f"country_blocked:{country}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                ...
                if (
                    ...
                    and country not in self.config.allowed_countries
                ):
                    self._log_blocked_access(
                        request, client_ip, f"country_not_allowed:{country}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail="Access denied - Geographic restriction",
        ...
        request.state.client_country = (
            self._get_country_code(client_ip) if self.geoip_reader else None
        )

...
from fastapi import FastAPI, HTTPException, UploadFile, File, Depends, BackgroundTasks, Request, WebSocket, WebSocketDisconnect
...
from schemas import (
    DocumentResponse, ChatRequest, ChatResponse, DocumentUpload, HealthResponse,
    DocumentSearchParams, DocumentListResponse, PaginationMetadata, DocumentType,
    DocumentStatus, SortField, SortOrder, PerformanceMetrics, IntelligenceFlags
)
from services.document_processor import document_processor  # Import the instance, not class
...
from auth_middleware import AuthenticationMiddleware, RateLimitMiddleware as AuthRateLimitMiddleware, get_current_user, get_current_organization
from organization_middleware import (
    OrganizationQueryFilter, OrganizationSecurityLogger, 
    OrganizationSecurityViolation, get_org_filtered_query
)
from logger import setup_logging, log_event, log_metric
import logging

# Production imports
from error_handler import global_exception_handler, with_error_handling, UserFriendlyError, ErrorCategory
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    await handler.handle_message(websocket, AnonymousUser(), AnonymousOrg(), message)
            ...
            logger.info(f"WebSocket client disconnected normally", extra={
                "user_id": anonymous_user_id,
                "organization_id": anonymous_org_id
            })
        except Exception as e:
...
async def websocket_status():
    ...
    return {
        ...
        "organization_connections": stats["organization_counts"].get(anonymous_org_id, 0),
        "organization_users": [],
...
async def health_check(db: Session = Depends(get_db)):
    ...
    return HealthResponse(
        status="healthy" if db_status == "connected" and memory_status != "critical" else "degraded", 
...
async def get_demo_status(
    ...
    return {
        ...
        "message": "Demo mode active - Using AI provider exclusively" if is_demo else "Production mode - Multiple AI providers available"
    }
...
async def upload_document(
    ...
    try:
        ...
        logger.info(f"Document upload initiated", extra={
        ...
        log_metric(logger, "document_uploaded", 1, unit="count", file_type=file_extension)
        ...
        background_tasks.add_task(process_document_background, doc_id, file_path, current_org.id)
        
        logger.info(f"Background processing scheduled", extra={
            "filename": file.filename,
            "document_id": doc_id,
            "organization_id": current_org.id
        })
        ...
        logger.error(f"Document upload failed", extra={
            "error": str(e),
            "user_id": current_user.id,
            "organization_id": current_org.id
        })
    ...
    try:
        ...
        document_responses = [
            DocumentResponse(
                ...
                metadata=json.loads(doc.legal_metadata) if getattr(doc, 'legal_metadata', None) else None
            )
        ...
        logger.error(f"Error fetching documents", extra={
            "error": str(e),
            "organization_id": current_org.id
        })
...
async def get_document(
    ...
    return DocumentResponse(
        ...
        content=getattr(document, 'extracted_content', None),  # Map extracted_content to content
        metadata=json.loads(document.legal_metadata) if getattr(document, 'legal_metadata', None) else None
    )
...
async def delete_document(
    ...
    try:
        ...
        logger.error(f"Error deleting document", extra={
...
async def search_documents(
    ...
    try:
        ...
        if search_params.min_risk_score is not None or search_params.max_risk_score is not None:
            ...
            if search_params.min_risk_score is not None:
                ...
                query = query.filter(
                    Document.legal_metadata.like(f'%"risk_score":%')
                )
            ...
            for doc in documents:
                if doc.legal_metadata:
                    try:
                        ...
                        if search_params.min_risk_score is not None and risk_score < search_params.min_risk_score:
                            continue
                        if search_params.max_risk_score is not None and risk_score > search_params.max_risk_score:
                            continue
        ...
        # Calculate pagination metadata
        total_pages = math.ceil(total_items / search_params.limit) if search_params.limit > 0 else 1
        current_page = (search_params.offset // search_params.limit) + 1 if search_params.limit > 0 else 1
        ...
        filters_applied = {
            ...
            "date_range": {
                "from": search_params.date_from.isoformat() if search_params.date_from else None,
                "to": search_params.date_to.isoformat() if search_params.date_to else None
            },
            "risk_score_range": {
        ...
        logger.error(f"Error searching documents", extra={
            "error": str(e),
            "organization_id": current_org.id
        })
    ...
    try:
        ...
        if not perf_metrics and "response_metrics" in response:
            ...
            perf_metrics = {
                ...
                "response_source": "metadata_cache" if old_metrics.get("response_type") == "instant_metadata" else "ai_analysis",
                "cost_savings_estimate": f"${old_metrics.get('cost_saved', 0):.3f}" if old_metrics.get('cost_saved') else "$0.00",
                "query_classification": "metadata_query" if old_metrics.get("response_type") == "instant_metadata" else "analysis_request"
            }
        ...
        ai_message = ChatMessage(
            ...
            processing_time=perf_metrics.get("total_response_time_ms") if perf_metrics else None
        )
        ...
        performance_metrics = PerformanceMetrics(
            ...
            query_classification=perf_metrics.get("query_classification", "conversational")
        )
        ...
        chat_response = ChatResponse(
            ...
            cost_saved=float(perf_metrics.get("cost_savings_estimate", "0").replace("$", "")) if perf_metrics.get("cost_savings_estimate") else 0
        )
        ...
        logger.error(f"Chat processing error", extra={
            "error": str(e),
            "organization_id": current_org.id,
...
async def semantic_search(
    ...
    try:
        ...
        # Log search
        logger.info("Semantic search performed", extra={
            "query": query[:100],
...
async def find_similar_documents(
    ...
    try:
        ...
        logger.info("Similar documents found", extra={
        ...
        logger.error("Find similar documents failed", extra={
            "error": str(e),
            "document_id": document_id,
            "user_id": current_user.id,
            "organization_id": current_org.id
...
async def rag_chat(
    ...
    try:
        ...
        rag_response = await rag_service.query(
            ...
            organization_id=current_org.id,
            document_ids=chat_request.document_ids if hasattr(chat_request, 'document_ids') else None,
            user_id=current_user.id,
            conversation_history=conversation_history
        )
        
        # Save to chat history
        session_id = chat_request.session_id if hasattr(chat_request, 'session_id') else str(uuid.uuid4())
        ...
        logger.info("RAG chat completed", extra={
            "session_id": session_id,
        ...
        logger.error("RAG chat failed", extra={
            "error": str(e),
            "user_id": current_user.id,
            "organization_id": current_org.id
        })
...
async def test_rag_chat(
    ...
    try:
        ...
        return {
            "response": rag_response.answer,
            "sources": [{"document_id": s.document_id, "title": s.title, "content": s.content, "relevance": s.relevance} for s in rag_response.sources],
            "confidence": rag_response.confidence,
...
async def list_mcp_servers(
    ...
    try:
        ...
        logger.error(f"MCP list servers error", extra={
            "error": str(e),
            "user_id": current_user.id,
            "organization_id": current_org.id
        })
...
async def connect_mcp_server(
    ...
    try:
        ...
        logger.info(f"MCP server connection", extra={
        ...
        logger.error(f"MCP connection failed", extra={
            "error": str(e),
            "server_name": server_name,
            "user_id": current_user.id,
            "organization_id": current_org.id
...
async def process_document_background(doc_id: str, file_path: str, organization_id: str = None):
    """Process document in background with organization context"""
    try:
        logger.info(f"Starting background processing for document", extra={
            "document_id": doc_id,
        ...
        if success:
            logger.info(f"Background processing completed successfully", extra={
                "document_id": doc_id,
                "organization_id": organization_id
            })
        else:
            logger.error(f"Background processing failed", extra={
                "document_id": doc_id,
                "organization_id": organization_id
            })
            
    except Exception as e:
        logger.error(f"Background processing error", extra={
            "document_id": doc_id,
            "organization_id": organization_id,
            "error": str(e)
        })
...
async def get_ai_preferences(
    ...
    return {
        "user_preferences": {
            ...
            "model_preferences": current_user.ai_model_preferences or {"openai": "gpt-4"},

            ...
            for column_name, column_type in doc_columns:
                if column_name not in existing_doc_columns:
                    ...
                    try:
                        ...
                        print(f"⚠️ Documents column {column_name} might already exist: {e}")
                else:
            ...
            try:
                ...
                for column_name, column_type in session_columns:
                    if column_name not in existing_session_columns:
                        ...
                        try:
                            conn.execute(text(f"ALTER TABLE chat_sessions ADD COLUMN {column_name} {column_type}"))
                            conn.commit()
                            print(f"✅ Added chat_sessions column: {column_name}")
                        except Exception as e:
                            print(f"⚠️ Chat_sessions column {column_name} might already exist: {e}")
                    else:
                        print(f"✅ Chat_sessions column {column_name} already exists")
                
                # If we have created_at but not created_timestamp, copy the data
                if 'created_at' in existing_session_columns and 'created_timestamp' not in existing_session_columns:
                    print("🔄 Copying created_at to created_timestamp...")
                    try:
                        conn.execute(text("UPDATE chat_sessions SET created_timestamp = created_at WHERE created_timestamp IS NULL"))
                        conn.commit()
                ...
                for column_name, column_type in message_columns:
                    if column_name not in existing_message_columns:
                        ...
                        try:
                            ...
                            print(f"⚠️ Chat_messages column {column_name} might already exist: {e}")
                    else:

...
class SecurityPolicyUpdate(BaseModel):
    classification: str = Field(..., pattern="^(public|internal|confidential|restricted|privileged)$")
...
async def get_compliance_score(
    ...
    return {
        ...
        "components": {
            ...
            ) / report["prompt_review_stats"]["total_prompts"] * 100
            if report["prompt_review_stats"]["total_prompts"] > 0 else 100
        }
    }
...
async def enforce_data_retention(
    ...
    background_tasks.add_task(
        service.enforce_data_retention_policy,
        db,
    ...
    pref = db.query(ConsentPreference).filter(
        ConsentPreference.organization_id == current_user.organization_id
    ).first()
        ...
        if log.decision_summary:
            ...
            if decision.get("action") in ["block", "require_review"]:
                incidents.append({
                    ...
                    "severity": "high" if decision.get("action") == "block" else "medium"
                })
...
async def get_security_status(
    ...
    blocked_count = db.query(func.count(AIAuditLog.id)).filter(
        AIAuditLog.organization_id == current_user.organization_id,
    ...
    pending_reviews = db.query(func.count(PromptLog.id)).filter(

...
class SecurityMonitor:
    ...
    def record_login_attempt(
        ...
        if not success:
            ...
            recent_failures = [
                a for a in self.failed_login_attempts[key]
                if not a["success"] and 
                datetime.utcnow() - a["timestamp"] < timedelta(minutes=15)
            ]
    ...
    def detect_data_exfiltration(
        ...
        recent_downloads = [
            ...
            if a.get("action") in ["download", "export"] and
            datetime.utcnow() - a["timestamp"] < timedelta(seconds=time_window)
        ]
    ...
    def analyze_user_behavior(
        ...
        if profile.typical_login_times and hour not in profile.typical_login_times:
            # Unusual login time
            avg_hour = sum(profile.typical_login_times) / len(profile.typical_login_times)
        ...
        if profile.typical_ip_addresses and ip_address not in profile.typical_ip_addresses:
    ...
    def get_threat_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            incidents = db.query(SecurityIncidentDB).filter(
                SecurityIncidentDB.timestamp > recent_cutoff
            ).all()
    ...
    def _trigger_security_alert(self, incident: SecurityIncident):
        ...
        logger.critical(f"SECURITY ALERT: {incident.threat_type} - {incident.description}")

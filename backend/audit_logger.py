...
class AuditLogger:
    ...
    def log_document_access(
        self, user: User, document_id: str, action: str, ip_address: str
    ):
        """Log document access"""
        event_type_map = {
    ...
    def log_data_export(
        self, user: User, export_type: str, resource_count: int, ip_address: str
    ):
            ...
            report = {
                ...
                "summary": {
                    ...
                    "data_export_requests": sum(
                        1
                        for e in events
                        if e.event_type == AuditEventType.DATA_EXPORT_REQUEST
                    ),
                    "data_deletion_requests": sum(
                        1
                        for e in events
                        if e.event_type == AuditEventType.DATA_DELETE_REQUEST
                    ),
                    "consent_given": sum(
                        1
                        for e in events
                        if e.event_type == AuditEventType.CONSENT_GIVEN
                    ),
                    "consent_withdrawn": sum(
                        1
                        for e in events
                        if e.event_type == AuditEventType.CONSENT_WITHDRAWN
                    ),
                },
                "events": [
                    {

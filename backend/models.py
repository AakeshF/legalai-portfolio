...
class Organization(Base):
    ...
    ai_budget_period_start = Column(
        DateTime, nullable=True
    )  # Start of current budget period
    ai_current_month_cost = Column(
        Float, default=0.0
...
class User(Base):
    ...
    # AI preferences
    ai_provider_preference = Column(
        String(50), nullable=True
    )  # claude, openai, gemini, local
    ai_model_preferences = Column(
        JSON, nullable=True
    )  # {"claude": "opus", "openai": "gpt-4"}
...
class Document(Base):
    ...
    legal_metadata = Column(
        Text, nullable=True
    )  # JSON string containing extracted legal metadata
...
class ChatMessage(Base):
    ...
    def __repr__(self):
        return (
            f"<ChatMessage(id={self.id}, session={self.session_id}, role={self.role})>"
        )
...
class Deadline(Base):
    ...
    # MCP integration
    mcp_sync_source = Column(
        String, nullable=True
    )  # court_calendar, client_calendar, etc.
...
class MCPQueryCache(Base):
    ...
    )  # legal_data, court_api, client_data
...
class ClientCommunication(Base):
    ...
    )  # email, phone, sms, meeting, letter
    direction = Column(String, nullable=False)  # inbound, outbound
    ...
    privilege_type = Column(
        String, default="not_privileged"
    )  # attorney_client, work_product, confidential
    ...
    integration_source = Column(
        String, nullable=True
    )  # email_server, phone_system, etc.
...
class CommunicationFollowUp(Base):
    ...
    communication_id = Column(
        String, ForeignKey("client_communications.id"), nullable=False
    )
...
Matter.client_communications = relationship(
    "ClientCommunication", back_populates="matter"
)
...
class MCPHealthStatus(Base):
    ...
    error = Column(String, nullable=True)
    health_metadata = Column(
        JSON, default=dict
    )  # capabilities, version, uptime_percentage
...
class MCPAlert(Base):
    ...
    type = Column(
        String, nullable=False
    )  # error_rate, slow_response, server_down, etc.
...
class MCPCacheMetrics(Base):
    ...
    event_type = Column(
        String, nullable=False, index=True
    )  # hit, miss, eviction, stale, refresh
...
class DocumentChunk(Base):
    ...
    chunk_metadata = Column(
        JSON
    )  # {"section": "3.2", "type": "clause", "headers": [...]}
...
class ChunkEmbedding(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    chunk_id = Column(
        String, ForeignKey("document_chunks.id", ondelete="CASCADE"), nullable=False
    )
...
class SearchCache(Base):
    ...
    def __repr__(self):
        return (
            f"<SearchCache(query_hash={self.query_hash[:8]}..., hits={self.hit_count})>"
        )

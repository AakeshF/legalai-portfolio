...
async def update_ai_backend_config(
    ...
    logger.info(
        f"AI backend configuration updated",
        extra={
...
async def test_ai_backend(
    ...
    try:
        ...
        logger.error(
            f"Backend test failed",
            extra={"backend": test_request.backend, "error": str(e)},
        )

        return {"success": False, "backend": test_request.backend, "error": str(e)}
        ...
        "usage": {
            "cloud": {
                ...
                "success_rate": (
                    (
                        (
                            cloud_metrics.get("requests", 0)
                            - cloud_metrics.get("errors", 0)
                        )
                        / cloud_metrics.get("requests", 1)
                        * 100
                    )
                    if cloud_metrics.get("requests", 0) > 0
                    else 0
                ),
            },
            "local": {
                ...
                "success_rate": (
                    (
                        (
                            local_metrics.get("requests", 0)
                            - local_metrics.get("errors", 0)
                        )
                        / local_metrics.get("requests", 1)
                        * 100
                    )
                    if local_metrics.get("requests", 0) > 0
                    else 0
                ),
            },
        },
...
async def check_backend_health(
    ...
    return {
        ...
        "last_check": (
            health_info["last_check"].isoformat() if health_info["last_check"] else None
        ),

...
@router.post(
    "/{document_id}/enhance-with-mcp", response_model=DocumentMCPEnhanceResponse
)
...
async def classify_document_with_mcp(
    request: DocumentClassifyRequest, current_user: User = Depends(get_current_user)
):
    """Classify a document using court-specific MCP knowledge"""
    try:
        # Perform classification
        classification = await classifier.classify_with_court_knowledge(
            document_text=request.document_text, metadata=request.metadata or {}
        )
...
async def bulk_enhance_documents(
    ...
    if not document_ids:
        ...
        unenhanced_docs = (
            ...
            .limit(request.max_documents or 50)
            .all()
        )
    ...
    if not document_ids:
        return BulkEnhanceResponse(
            total_documents=0, queued=0, message="No documents to enhance"
        )

    # Queue enhancement tasks
    background_tasks.add_task(enhance_documents_batch, document_ids, organization_id)
...
async def get_mcp_enhancement_stats(
    ...
    # Get total documents
    total_docs = (
        db.query(Document).filter(Document.organization_id == organization_id).count()
    )
    ...
    return {
        ...
        "enhancement_percentage": (
            (enhanced_docs / total_docs * 100) if total_docs > 0 else 0
        ),
        "enhancement_types": {
...
async def enhance_documents_batch(document_ids: List[str], organization_id: str):
    ...
    try:
        ...
        logger.info(
            f"Batch enhancement complete: {success_count} success, {error_count} errors"
        )
...
async def extract_legal_concepts_from_query(
    ...
    try:
        ...
        return {"query": query, "concepts": concepts}

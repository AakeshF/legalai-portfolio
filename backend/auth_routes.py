...
from frontend_auth_adapter import (
    FrontendRegisterRequest,
    FrontendLoginResponse,
    FrontendRefreshRequest,
)
...
async def register_frontend(
    ...
    try:
        ...
        if isinstance(request, FrontendRegisterRequest) or hasattr(
            request, "full_name"
        ):
...
async def register_backend(request: RegisterRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        tokens = create_user_tokens(
            admin_user.id, admin_user.email, admin_user.organization_id, admin_user.role
        )

        # Send welcome email
        await send_welcome_email(
            admin_user.email, admin_user.full_name, organization.name
        )
...
async def refresh_token(
    body: Union[FrontendRefreshRequest, Dict] = Body(...), db: Session = Depends(get_db)
):
...
async def reset_password(token: str, new_password: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        user = (
            db.query(User)
            .filter(User.id == payload.get("sub"), User.email == payload.get("email"))
            .first()
        )

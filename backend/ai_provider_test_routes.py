    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            if provider == 'openai':
                async with session.get(test_url, headers=headers) as response:
                    ...
                    if response.status == 200:
                        ...
                        if 'x-ratelimit-remaining-requests' in response.headers:
                            billing_info = {
                                'quotaRemaining': int(response.headers.get('x-ratelimit-remaining-requests', 0)),
                                'quotaLimit': int(response.headers.get('x-ratelimit-limit-requests', 0))
                            }
                        ...
                        if "quota" in error_text.lower() or "billing" in error_text.lower():
                            return AuthTestResponse(
                                authenticated=True,
                                healthy=False,
                ...
                async with session.post(f"{base_url}/messages", headers=headers, json=test_payload) as response:
                    response_time = int((time.time() - start_time) * 1000)

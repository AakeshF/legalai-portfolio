...
async def verify_two_factor(
    ...
    try:
        result = two_factor_service.verify_and_enable_2fa(
            current_user, totp_code, trust_device, device_id
        )
...
async def get_security_incidents(
    ...
    incidents = query.order_by(SecurityIncidentDB.timestamp.desc()).limit(limit).all()
...
async def validate_file_upload(
    ...
    is_valid, details = await file_validator.validate_upload(
        file, current_user.id, current_org.id
    )

    if not is_valid:
        raise HTTPException(
            status_code=400,
            detail={"message": "File failed security validation", "details": details},
        )
...
async def get_encryption_status(
    ...
    total_docs = (
        db.query(Document).filter(Document.organization_id == current_org.id).count()
    )
    ...
    return {
        ...
        "documents": {
            ...
            "percentage": (
                (encrypted_docs / total_docs * 100) if total_docs > 0 else 100
            ),
        },
        "key_rotation": {
...
async def generate_security_audit_report(
    ...
    login_stats = {
        "successful_logins": sum(
            1 for e in login_events if e.event_type == "auth.login.success"
        ),
        "failed_logins": sum(
            1 for e in login_events if e.event_type == "auth.login.failure"
        ),

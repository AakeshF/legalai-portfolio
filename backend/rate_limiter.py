...
class RateLimiter:
        ...
        if key in self.penalties:
            ...
            if datetime.utcnow() < penalty_end:
                ...
                return False, retry_after, {
                    "reason": "penalty",
                    "penalty_ends": penalty_end.isoformat()
                }
            else:
                # Penalty expired
    ...
    def check_suspicious_activity(self, request: Request, user_id: Optional[str] = None) -> bool:
        """
        ...
        endpoint_history[:] = [e for e in endpoint_history if datetime.fromisoformat(e["timestamp"]) > cutoff]
        ...
        if request.url.path == "/api/auth/login" and user_id is None:
            ...
            # Clean old entries
            auth_history[:] = [e for e in auth_history if datetime.fromisoformat(e["timestamp"]) > cutoff]
        ...
        if self._is_automated_tool(user_agent):
            ...
            if tool_key not in self.suspicious_activity:
                logger.warning(f"Automated tool detected: {user_agent} from {ip_address}")
                self.suspicious_activity[tool_key] = [{"timestamp": datetime.utcnow().isoformat()}]
            return True
    ...
    def _is_automated_tool(self, user_agent: str) -> bool:
        ...
        suspicious_patterns = [
            ...
            "python-requests", "postman", "insomnia", "scanner"
        ]
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self.rate_limiter.check_suspicious_activity(request, user_id):
            ...
            if self.rate_limiter.audit_logger:
                ...
                self.rate_limiter.audit_logger.log_event(AuditEvent(

...
class ErrorHandler:
    ...
    def get_user_message(category: str, error_type: str = "default", **kwargs) -> str:
        ...
        messages = ErrorHandler.ERROR_MESSAGES.get(category, ErrorHandler.ERROR_MESSAGES[ErrorCategory.INTERNAL])
        ...
        if isinstance(error, OperationalError):
            if "connection" in str(error).lower():
                return UserFriendlyError(
                    status_code=503,
                    user_message=ErrorHandler.get_user_message(ErrorCategory.DATABASE, "connection"),
                ...
                return UserFriendlyError(
                    status_code=504,
                    user_message=ErrorHandler.get_user_message(ErrorCategory.DATABASE, "timeout"),
                    technical_details=str(error),
                    error_category=ErrorCategory.DATABASE,
                    retry_after=10
                )
            ...
            return UserFriendlyError(
                status_code=409,
                user_message=ErrorHandler.get_user_message(ErrorCategory.DATABASE, "integrity"),
                technical_details=str(error),
                error_category=ErrorCategory.DATABASE
            )
    ...
    @staticmethod
    def handle_http_error(error: httpx.HTTPError, service_name: str = "external") -> UserFriendlyError:
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                raise UserFriendlyError(
                    status_code=500,
                    user_message=user_message or ErrorHandler.get_user_message(category),
...
class RetryConfig:
    ...
    def __init__(
        ...
        self.retry_on = retry_on or [httpx.TimeoutException, httpx.ConnectError, OperationalError]

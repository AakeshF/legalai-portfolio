...
class EncryptionService:
    ...
    def __init__(self, master_key: Optional[str] = None):
        ...
        if master_key:
            ...
            if env_key:
                ...
                logger.warning(
                    "No master key provided. Generating new key - THIS SHOULD BE STORED SECURELY!"
                )
                self.master_key = self._generate_master_key()
                logger.info(
                    f"Generated master key (store this securely): {base64.b64encode(self.master_key).decode()}"
                )
    ...
    def encrypt_document(
        self, content: bytes, document_id: str
    ...
    def decrypt_document(self, encrypted_content: bytes, document_id: str) -> bytes:
        ...
        try:
            ...
            logger.info(
                ...
                extra={"document_id": document_id, "decrypted_size": len(plaintext)},
            )
            ...
            logger.error(
                f"Decryption failed for document",
                extra={"document_id": document_id, "error": str(e)},
            )
            raise ValueError(
                "Decryption failed. Document may be corrupted or tampered with."
            )
    ...
    def verify_encryption_integrity(
        self, encrypted_content: bytes, metadata: Dict[str, Any]
    ) -> bool:
        """

...
class TwoFactorService:
    ...
    def __init__(self, db_session_factory, config: TwoFactorConfig = None, audit_logger: AuditLogger = None):
    ...
    def setup_2fa(self, user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            secret_encrypted = self.encryption_service.encrypt_field(secret, f"totp_secret_{user.id}")
            backup_codes_encrypted = self.encryption_service.encrypt_field(
                json.dumps(backup_codes),
                f"backup_codes_{user.id}"
            )
    ...
    def verify_and_enable_2fa(
        ...
        try:
            ...
            secret = self.encryption_service.decrypt_field(
                two_fa.secret_encrypted,
                f"totp_secret_{user.id}"
            )
    ...
    def verify_totp(
        ...
        try:
            ...
            if two_fa.locked_until and datetime.utcnow() < two_fa.locked_until:
                remaining = int((two_fa.locked_until - datetime.utcnow()).total_seconds())
            ...
            if device_token and self._is_device_trusted(two_fa, device_token):
                logger.info(f"2FA bypassed for trusted device", extra={
                    "user_id": user.id
                })
                return True, None
            ...
            if two_fa.backup_codes_encrypted:
                codes = json.loads(
                    self.encryption_service.decrypt_field(
                        two_fa.backup_codes_encrypted,
                        f"backup_codes_{user.id}"
                    )
                )
                backup_codes_count = len([c for c in codes if c])  # Count non-null codes
    ...
    def _verify_backup_code(self, two_fa: TwoFactorAuth, code: str, db: Session) -> bool:
        ...
        codes = json.loads(
            self.encryption_service.decrypt_field(
                two_fa.backup_codes_encrypted,
                f"backup_codes_{two_fa.user_id}"
            )
        )
    ...
    def _trust_device(self, two_fa: TwoFactorAuth, device_id: str) -> str:
        ...
        if two_fa.trusted_devices:
            trusted_devices = json.loads(
                self.encryption_service.decrypt_field(
                    two_fa.trusted_devices,
                    f"trusted_devices_{two_fa.user_id}"
                )
            )

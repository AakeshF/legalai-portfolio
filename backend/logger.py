...
def log_event(logger: logging.Logger, event_type: str, message: str, **data):
    ...
    logger.info(message, extra={"event_type": event_type, "event_data": data})


# Log metrics
def log_metric(
    logger: logging.Logger, metric_name: str, value: float, unit: str = "count", **tags
):
...
setup_logging(
    ...
    log_format=os.environ.get(
        "LOG_FORMAT",
        "json" if os.environ.get("ENVIRONMENT") == "production" else "simple",
    ),
)

        ...
        try:
            ...
            return {
                ...
                "connection_pool": {
                    ...
                    "checked_out": (
                        db.bind.pool.checkedout() if hasattr(db.bind, "pool") else "N/A"
                    ),
                },
...
async def startup_probe() -> Dict[str, Any]:
    ...
    checks = {"database": False, "configuration": False, "migrations": False}
    ...
    if not all_ready:
        raise HTTPException(
            status_code=503, detail={"status": "starting", "checks": checks}
        )
...
async def feature_health_check() -> Dict[str, Any]:
    ...
    features = {
        ...
        "email_notifications": {
            "enabled": os.getenv("ENABLE_EMAIL_NOTIFICATIONS", "true").lower()
            == "true",
    ...
    return {"timestamp": datetime.now(timezone.utc).isoformat(), "features": features}
